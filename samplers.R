  ...
  run = function() {
    ...
    if(startmidfzt<=endmidfzt){
      for(zt in startmidfzt:endmidfzt){
        ...
        #calc filtered prob
        q2 <- 1/(1+exp(lf[zt,1]+log(p[1,1])-lf[zt,2]-log(p[2,1])+model$mup[loc,ct]))
        q[zt,1:2] <<- c(1-q2,q2)
      }
    }
    ...
    if(startbs==Mt){
      ...
      p <- t(tm) %*% asCol(q_tm1[1:2])
      q2 <- 1/(1+exp(log(p[1,1])-log(p[2,1])+model$mup[loc,Mt]))
  ...
  run = function() {
    ...
    if(startmidfzt<=endmidfzt){
      for(zt in startmidfzt:endmidfzt){
        ...
        if(neilocs==1){
          #(0,0)
          p1100 <- expit(model$lp01[loc1,ct]+(0-s2m)*model$alpha[6])
          p2100 <- expit(model$lp01[loc2,ct]+(0-s1m)*model$alpha[6])
          #(1,0)
          p1110 <- expit(model$lp11[loc1,ct]+(0-s2m)*model$alpha[12])
          p2110 <- expit(model$lp01[loc2,ct]+(1-s1m)*model$alpha[6])
          #(0,1)
          p1101 <- expit(model$lp01[loc1,ct]+(1-s2m)*model$alpha[6])
    ...
    if(startbs==Mt){
      ...
      if(neilocs==1){
        #(0,0)
        p1100 <- expit(model$lp01[loc1,Mt]+(0-s2m)*model$alpha[6])
        p2100 <- expit(model$lp01[loc2,Mt]+(0-s1m)*model$alpha[6])
        #(1,0)
        p1110 <- expit(model$lp11[loc1,Mt]+(0-s2m)*model$alpha[12])
        p2110 <- expit(model$lp01[loc2,Mt]+(1-s1m)*model$alpha[6])
        #(0,1)
        p1101 <- expit(model$lp01[loc1,Mt]+(1-s2m)*model$alpha[6])
    ...
    for(izt in startizt:(numtimes)){
      ...
      if(neilocs==1){
        #(0,0)
        p1100 <- expit(model$lp01[loc1,ct+1]+(0-s2m)*model$alpha[6])
        p2100 <- expit(model$lp01[loc2,ct+1]+(0-s1m)*model$alpha[6])
        #(1,0)
        p1110 <- expit(model$lp11[loc1,ct+1]+(0-s2m)*model$alpha[12])
        p2110 <- expit(model$lp01[loc2,ct+1]+(1-s1m)*model$alpha[6])
        #(0,1)
        p1101 <- expit(model$lp01[loc1,ct+1]+(1-s2m)*model$alpha[6])
