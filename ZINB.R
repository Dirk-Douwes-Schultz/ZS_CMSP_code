##ZINB model from Table 1 of the main text
rm(list=ls())
library(nimble)
...
dengeeCode <- nimbleCode({
  ...
  for(i in 1:N) {
    ...
    for(t in 2:T){
      ...
      logit(p1[i,t]) <-  alpha[1]+alpha[2]*(pop[i]-mean(pop[]))+
        alpha[3]*(HDI[i]-mean(HDI[]))+alpha[4]*(Temp[t-1]-mean(Temp[]))+
        alpha[5]*(Rain[t-1]-mean(Rain[]))+alpha[6]*(lpsi[i,t-1]-mlpsi)
...
inits <- list(beta0 = rnorm(n=1,mean=0,sd=1),beta1 = rnorm(n=1,mean=0,sd=.01),
              ...
              alpha=rnorm(n=6,mean=0,sd=c(.1,.01,1,.1,.01,.01)),
...
dengeeConf$removeSampler(c("alpha[1]","alpha[2]"))
dengeeConf$addSampler(target = c("alpha[1]","alpha[2]"),type = "AF_slice")
...
plot(samples[,c("alpha[5]","alpha[6]")])
