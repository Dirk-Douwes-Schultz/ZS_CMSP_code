##ZINBRE model from Table 1 of the main text
rm(list=ls())
library(nimble)
...
dengeeConsts <- list(N=160,T=84,adj=as.numeric(as.carAdjacency(N_matrix)$adj),
                     ...
                     L=length(as.numeric(as.carAdjacency(N_matrix)$adj)))
...
dengeeCode <- nimbleCode({
  ...
  bs[1:N]~dcar_proper(mus[1:N], adj=adj[1:L], num=num[1:N], tau=tau, 
                      gamma=gamma)
  ...
  for(i in 1:N) {
    ...
    for(t in 2:T){
      ...
      logit(p1[i,t]) <-  bs[i]+alpha[1]+alpha[2]*(pop[i]-mean(pop[]))+
        alpha[3]*(HDI[i]-mean(HDI[]))+alpha[4]*(Temp[t-1]-mean(Temp[]))+
        alpha[5]*(Rain[t-1]-mean(Rain[]))+alpha[6]*(lpsi[i,t-1]-mlpsi)+alpha[7]*S[i,t-1]
...
inits <- list(beta0 = rnorm(n=1,mean=0,sd=1),beta1 = rnorm(n=1,mean=0,sd=.01),
              ...
              alpha=rnorm(n=7,mean=0,sd=c(.1,.01,1,.1,.01,.01,.1)),
              b=rnorm(n=160,mean=0,sd=.01),b0=rnorm(n=160,mean=0,sd=.01),
              lr=runif(n=160,min=-10,max=10),bs=rnorm(n=160,mean=0,sd=.5),
              tau=4,gamma=runif(n=1,-1,1))
...
dengeeConf$removeSampler(c("alpha[1]","alpha[2]","alpha[7]"))
dengeeConf$addSampler(target = c("alpha[1]","alpha[2]","alpha[7]"),type = "AF_slice")

print(dengeeConf)

dengeeConf$addMonitors(c("beta0","rho" ,"b","sigma_b","alpha","beta1","beta2","r","b0","sigma_b0","bs"))
...
start_time <- Sys.time()
samples <- runMCMC(CdengeeMCMC, niter = 150000,nchains = 3,nburnin=50000
...
gelman.diag(samples[,c("bs[1]","bs[2]","bs[3]","bs[4]",
                       "bs[21]","bs[39]","bs[112]","bs[55]",
                       "bs[23]","bs[29]","bs[15]","bs[74]",
                       "bs[41]","bs[123]","bs[16]","bs[59]")])
...
plot(samples[,c("tau","gamma","bs[1]")])
plot(samples[,c("b[4]","b[143]","b[50]","b[100]")])
plot(samples[,c("b0[4]","b0[143]","b0[50]","b0[100]")])
plot(samples[,c("bs[4]","bs[143]","bs[50]","bs[100]")])
plot(samples[,c("bs[2]","bs[3]","bs[5]","bs[6]")])
...
#WAIC = 67,825, 67,782 if alpha[7] (the second autoregressive effect) is removed
#we report 67,782 in Table 1 for a fair comparison as alpha[7] is not significant 
